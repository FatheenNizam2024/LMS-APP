...
const groupSubmissions = (submissions: any[], allCourses: any[]) => {
  ...
  return submissions.reduce((acc, sub) => {
    ...
    if (!acc[userId].courses[sub.courseId]) { acc[userId].courses[sub.courseId] = { courseName: course.name, moduleAssignments: [], finalAssignments: [], }; }
    let assignmentTitle = "Unknown Assignment";
    let isFinal = true;
    for (const module of (course.modules || [])) {
        const foundAssignment = (module.assignments || []).find((a:any) => a.assignmentId.toString() === sub.assignmentId.toString());
        if (foundAssignment) { assignmentTitle = foundAssignment.title; isFinal = false; break; }
    }
    if (isFinal) {
        const foundFinal = (course.finalAssignments || []).find((a:any) => a.assignmentId.toString() === sub.assignmentId.toString());
        if (foundFinal) assignmentTitle = foundFinal.title;
    }
    const submissionData = { ...sub, title: assignmentTitle, link: sub.content.url, };
    if (isFinal) { acc[userId].courses[sub.courseId].finalAssignments.push(submissionData); } 
    else { acc[userId].courses[sub.courseId].moduleAssignments.push(submissionData); }
    return acc;
  }, {});
};
  ...
  return (
    <div className="mt-[120px] p-5">
      ...
      {Object.keys(groupedData).length > 0 ? (
        ...
        <Typography className="text-center mt-10">No assignment submissions found.</Typography>
      )}
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
          <Box className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[500px] bg-white dark:bg-slate-900 rounded-lg shadow-xl p-6">
              ...
              <FormControl fullWidth margin="normal">
                ...
                {/* MODIFICATION: Use lowercase values to match the schema */}
...
const SubmissionItem: FC<{submission: any, onGrade: (sub: any) => void, onDelete: (id: string) => void}> = ({ submission, onGrade, onDelete }) => (
    <Box className="ml-4 mt-2 p-2 rounded bg-white dark:bg-slate-800/50 flex justify-between items-center flex-wrap">
        <Box className="flex-grow">
            ...
            {/* MODIFICATION: Use lowercase values for status checking */}
