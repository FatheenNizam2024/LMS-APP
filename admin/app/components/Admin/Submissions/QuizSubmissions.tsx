// C:\LMS App copy Part 2\Lms-App - Copy\admin\app\components\Admin\Submissions\QuizSubmissions.tsx
...
// MODIFICATION: Standardized the return shape to always include an optional lessonTitle
const findQuizContext = (courses: any[], quizId: string): { courseName: string; moduleTitle: string; lessonTitle: string | null; quiz: any | null } => {
    if (!courses || !quizId) return { courseName: "N/A", moduleTitle: "N/A", lessonTitle: null, quiz: null };
    for (const course of courses) {
        const checkQuizzes = (quizList: any[], moduleTitle = "Final Quiz", lessonTitle: string | null = null) => {
            const found = quizList.find((q: any) => q.quizId === quizId);
            if (found) return { courseName: course.name, moduleTitle, lessonTitle, quiz: found };
            return null;
        }
        
        let result = checkQuizzes(course.finalQuizzes || []);
        if (result) return result;

        for (const module of course.modules) {
            result = checkQuizzes(module.quizzes || [], module.title);
            if (result) return result;
            for (const lesson of module.lessons) {
                result = checkQuizzes(lesson.quizzes || [], module.title, lesson.title);
                if (result) return result;
            }
        }
    }
    return { courseName: "N/A", moduleTitle: "N/A", lessonTitle: null, quiz: null };
};
...
const QuizSubmissions: FC<Props> = ({ courseId }) => {
  ...
  const groupedData = useMemo(() => {
      const groupQuizSubmissions = (submissions: any[], allCourses: any[]) => {
          ...
          return submissions.reduce((acc, sub) => {
              ...
              if (!acc[userId]) { acc[userId] = { userName: sub.userId.name, courses: {} }; }
              const course = allCourses.find(c => c._id === sub.courseId);
              if (!course) return acc;
              if (!acc[userId].courses[sub.courseId]) { acc[userId].courses[sub.courseId] = { courseName: course.name, modules: {}, finalQuizzes: [] }; }
              const { quiz, moduleTitle, lessonTitle } = findQuizContext(allCourses, sub.quizId);
              if (!quiz) return acc;
              const submissionData = { ...sub, quizTitle: quiz.title };
              if (moduleTitle === 'Final Quiz') {
                  acc[userId].courses[sub.courseId].finalQuizzes.push(submissionData);
              } else {
                  const moduleId = moduleTitle;
                  if (!acc[userId].courses[sub.courseId].modules[moduleId]) { acc[userId].courses[sub.courseId].modules[moduleId] = { moduleTitle, lessonQuizzes: {}, moduleQuizzes: [] }; }
                  if (lessonTitle) {
                      const lessonId = lessonTitle;
                      if (!acc[userId].courses[sub.courseId].modules[moduleId].lessonQuizzes[lessonId]) { acc[userId].courses[sub.courseId].modules[moduleId].lessonQuizzes[lessonId] = { lessonTitle, quizzes: [] }; }
                      acc[userId].courses[sub.courseId].modules[moduleId].lessonQuizzes[lessonId].quizzes.push(submissionData);
                  } else {
                      acc[userId].courses[sub.courseId].modules[moduleId].moduleQuizzes.push(submissionData);
                  }
              }
              return acc;
          }, {});
      ...
      if(data?.submissions && coursesData?.courses){
          return groupQuizSubmissions(data.submissions, coursesData.courses);
      }
      return {};
  }, [data, coursesData]);
  ...
  const handleUpdateScore = () => {
      if (editScore.id) {
          updateQuizSubmissionScore({ submissionId: editScore.id, score: editScore.value });
      }
  };
  ...
  const detailedQuizData = selectedSubmission ? findQuizContext(coursesData?.courses, selectedSubmission.quizId) : null;
  ...
  return (
    <div className="mt-[120px] p-5">
      ...
      {Object.keys(groupedData).length > 0 ? (
        ...
        <Typography className="text-center mt-10">No quiz submissions found.</Typography>
      )}

      {selectedSubmission && detailedQuizData && detailedQuizData.quiz && (
        <Modal open={open} onClose={() => setOpen(false)}>
            <Box className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[800px] h-[80vh] bg-white dark:bg-slate-900 rounded-lg shadow-xl p-6 overflow-y-auto">
                ...
                <p><strong>Student:</strong> {selectedSubmission.userId.name}</p>
                <p><strong>Course:</strong> {detailedQuizData.courseName}</p>
                {/* MODIFICATION: Check if lessonTitle exists before rendering */}
                <p><strong>Section:</strong> {detailedQuizData.moduleTitle}{detailedQuizData.lessonTitle && ` > ${detailedQuizData.lessonTitle}`}</p>
                <p><strong>Quiz:</strong> {detailedQuizData.quiz.title}</p>
                <Box className="flex items-center gap-2">
                    <strong>Score:</strong>
                    {editScore.id === selectedSubmission._id ? (
                        <>
                           ...
                           <Typography>/ {selectedSubmission.totalQuestions}</Typography>
                        ...
                        <>
                           <p>{selectedSubmission.score} / {selectedSubmission.totalQuestions}</p>
                           <IconButton size="small" onClick={() => setEditScore({ id: selectedSubmission._id, value: selectedSubmission.score })}>
                ...
                {detailedQuizData.quiz.questions.map((q: any, index: number) => {
                    const userAnswer = selectedSubmission.answers.find((a: any) => a.questionId === q._id);
                    return (
