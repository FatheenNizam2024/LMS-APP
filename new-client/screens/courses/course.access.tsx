...
import {
  View, Text, ScrollView, TouchableOpacity, StyleSheet, Linking, ActivityIndicator, TextInput, RefreshControl
} from "react-native";
...
export default function CourseAccessScreen() {
  const { lesson: lessonString, course: courseString } = useLocalSearchParams();
  
  const lesson: LessonType | null = lessonString ? JSON.parse(lessonString as string) : null;
  const course: CoursesType | null = courseString ? JSON.parse(courseString as string) : null;
  ...
  const { submission, loading, doRefetch } = useGetSubmission(assignment?.assignmentId!);
  ...
  const handleAssignmentSubmit = async () => {
    ...
    await axios.post(`${SERVER_URI}/submit-assignment`, submissionData, {
  ...
  return (
    <ScrollView 
        ...
        refreshControl={<RefreshControl refreshing={loading} onRefresh={doRefetch} />}
    >
      {lesson.videoUrl ? (
        <View style={styles.videoContainer}>
            <WebView source={{ uri: lesson.videoUrl }} allowsFullscreenVideo={true} />
        </View>
      ) : (
        <View style={styles.noVideoContainer}>
            <FontAwesome name="file-text-o" size={50} color="#ccc" />
            <Text style={styles.noVideoText}>Assignment Overview</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
    container: { flex: 1, padding: 10, backgroundColor: '#F6F7F9' },
