// C:\LMS App copy Part 2\Lms-App - Copy\server\controllers\user.controller.ts
...
export const updateUserEnrollment = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const courseExistInUser = userCourses.some(
            (c: any) => c.toString() === courseId
        );

        if (courseExistInUser) {
            user.courses = userCourses.filter(
                (c: any) => c.toString() !== courseId
            );
        ...
        // MODIFICATION: Invalidate course caches after updating enrollment
